...
import {Link, useNavigate} from "react-router-dom"; // ۱. ایمپورت useNavigate اصلاح شد
import Navbar from "../components/Navbar.jsx";
import axios from "axios";

const ForgotPassword = () => {
    const navigate = useNavigate(); // ۲. تعریف هوک نیویگیت اضافه شد
    const [identifier, setIdentifier] = useState("");
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    const [success, setSuccess] = useState("");

    // بقیه کدها کاملاً درست و بدون تغییر...
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setTimeout(() => {
                    // فرستادن لینک به صفحه بعدی برای نمایش به کاربر
                    navigate('/reset-done', {state: {resetLink: response.data.reset_link}});
                }, 1000);
            }
        } catch (err) {
