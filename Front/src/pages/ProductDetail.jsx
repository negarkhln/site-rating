...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // const ratingRes = await axios.get(`/api/user-rating/${id}/`);
        // if (ratingRes.data) setUserRating(ratingRes.data);

        // const commentsRes = await axios.get(`/api/comments/${id}/`);
        // setComments(commentsRes.data);

        // const watchlistRes = await axios.get(`/api/watchlist/${id}/`);
        // if (watchlistRes.data) setWatchlistItem(watchlistRes.data);
        ...
        setProduct({
          ...
          description:
            "یک دزد ماهر که قادر به نفوذ به رویاهای افراد است، فرصتی برای پاک کردن گذشته خود پیدا می‌کند.",
          storyline:
            "دوم کاب (لئوناردو دیکاپریو) یک دزد ماهر است که با نفوذ به رویاهای افراد، اسرار آنها را می‌دزدد. او حالا فرصتی برای بازگشت به خانه و فرزندانش پیدا می‌کند، به شرطی که بتواند ایده‌ای را در ذهن یک تاجر قدرتمند植入 کند.",
          poster: null,
          views_count: 89234,
        ...
        setComments([
          {
            ...
            text: "فیلم فوق‌العاده‌ای بود! بازیگری عالی و داستان جذاب.",
          ...
          },
          {
            ...
            text: "بسیار پیچیده و هوشمندانه. ارزش چندبار دیدن رو داره.",
            created_at: "2025-05-20 10:15",
            updated_at: "2025-05-21 09:00",
          },
        ]);
  ...
  const handleWatchlistRemove = async () => {
    if (!window.confirm("آیا از حذف این محصول از لیست تماشا مطمئن هستید؟"))
      return;
    try {
      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="p-8">
            ...
            {/* Download Button */}
            {(product.download_url || product.download_file) && (
