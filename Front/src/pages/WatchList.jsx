...
const Watchlist = () => {
    ...
    useEffect(() => {
        const fetchWatchlist = async () => {
            ...
            setLoading(true);
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/watchlist/', config);
    ...
    const handleStatusUpdate = async (productId, newStatus) => {
        ...
        try {
            await axios.post(`http://127.0.0.1:8000/api/watchlist/${productId}/`, {status: newStatus}, config);
            ...
            if (updatedItem) {
                if (newStatus === "favorite") newFavorites = [...newFavorites, updatedItem]; else if (newStatus === "watching") newWatching = [...newWatching, updatedItem]; else if (newStatus === "planning") newPlanning = [...newPlanning, updatedItem]; else if (newStatus === "completed") newCompleted = [...newCompleted, updatedItem];
            }
    ...
    const handleRemove = async (productId) => {
        if (!window.confirm("آیا از حذف این آیتم از لیست تماشا مطمئن هستید؟")) return;

        const config = getAuthConfig();
        if (!config) return;

        try {
            await axios.delete(`http://127.0.0.1:8000/api/watchlist/${productId}/`, config);
    ...
    const WatchlistSection = ({title, icon, items, statusType, bgColor}) => {
        ...
        return (<div className="bg-white rounded-xl shadow-md p-6 mb-6">
                ...
                {items.map((item) => (<div key={item.id}
                    ...
                    <div className="p-4">
                        <Link to={`/movie/${item.product.id}`}>
                            <h3 className="font-bold text-[#1A2A4A] text-lg mb-2 hover:text-[#C9A84C] transition">{item.product.Pname}</h3>
                        </Link>
                        {item.product.genre && <p className="text-gray-500 text-xs mb-3">{item.product.genre}</p>}
                        <div className="flex flex-wrap gap-2 mt-3">
                            <select value={statusType}
                                    onChange={(e) => handleStatusUpdate(item.product.id, e.target.value)}
    ...
    const hasItems = favorites.length > 0 || watching.length > 0 || planning.length > 0 || completed.length > 0;
    ...
    return (<div className="min-h-screen bg-[#F5F0E8] font-sans" dir="rtl">
        ...
        <div className="container mx-auto px-6 py-8">
            ...
            <WatchlistSection title="علاقه‌مندی‌های من" icon="❤️" items={favorites} statusType="favorite"
                              bgColor="bg-red-500"/>
            <WatchlistSection title="در حال تماشا" icon="🎬" items={watching} statusType="watching"
                              bgColor="bg-blue-500"/>
            <WatchlistSection title="بعداً می‌بینم" icon="📌" items={planning} statusType="planning"
                              bgColor="bg-purple-500"/>
            <WatchlistSection title="تماشا شده" icon="✅" items={completed} statusType="completed"
                              bgColor="bg-green-500"/>

            {!hasItems && (<div className="bg-white rounded-2xl shadow-md p-12 text-center">
                <div className="text-6xl mb-4">📭</div>
                <p className="text-gray-500 text-lg mb-2">لیست تماشای شما خالی است</p>
                <p className="text-gray-400 text-sm mb-6">برای افزودن، به صفحه جزئیات محصولات بروید و روی
                    "افزودن به لیست تماشا" کلیک کنید</p>
                <Link to="/movies"
